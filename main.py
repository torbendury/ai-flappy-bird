...
BIRD_IMAGES = [
    ...
    pygame.transform.scale2x(
        load_image(IMAGES_DIR_PREFIX, "bird1.png")
    ),  # wings normal
...
class Bird:
    ...
    def move(self):
        ...
        ):  # check if we are still in a jump motion or starting to fall
...
class Pipe:
    ...
    def set_height(self):
        ...
        )  # we possibly draw the pipe starting in a negative location, this is wanted
...
def draw_window(
    window: pygame.Surface, bird: Bird, pipes: list[Pipe], base: Base, score
):
    window.blit(
        BG_IMAGE, (0, 0)
    )  # TODO: Should be a moving object, moving muuuuuch slower than the rest so we have some kind of "journey"
